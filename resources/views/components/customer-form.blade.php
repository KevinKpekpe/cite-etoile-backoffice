...
<div class="form-grid">
    ...
    <label class="form-field"><span class="form-field__label">Statut</span><select name="status" class="form-select">@foreach(['prospect'=>'Prospect','active'=>'Actif','settled'=>'Soldé','suspended'=>'Suspendu'] as $value=>$label)<option value="{{ $value }}" @selected(old('status',$customer?->status ?? 'active')===$value)>{{ $label }}</option>@endforeach</select></label>
    ...
    <label class="form-field form-grid__wide"><span class="form-field__label">Adresse</span><textarea name="address" rows="2" class="form-control">{{ old('address',$customer?->address) }}</textarea></label>
