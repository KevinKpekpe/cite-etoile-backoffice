<x-layouts.app title="Mon profil">
    <div class="form-page portal-page">
        ...
        <form method="POST" action="{{ route('portal.profile.update') }}" class="form-page__content">
            @csrf
            @method('PUT')
            <section class="form-section">
                <div class="form-section__header"><span class="form-section__number">01</span><div><h2>Identité et contact</h2><p>Coordonnées utilisées pour vos contrats et communications.</p></div></div>
                <div class="form-section__body"><div class="form-grid form-grid--three">
                    @foreach(['first_name' => 'Prénom', 'last_name' => 'Nom', 'middle_name' => 'Postnom', 'phone' => 'Téléphone', 'secondary_phone' => 'Téléphone secondaire', 'whatsapp' => 'WhatsApp', 'email' => 'E-mail', 'commune' => 'Commune', 'city' => 'Ville', 'country' => 'Pays'] as $field => $label)
                        <label class="form-field"><span class="form-field__label">{{ $label }}@if(in_array($field, ['first_name', 'last_name', 'phone']))<span class="form-required">*</span>@endif</span><input name="{{ $field }}" value="{{ old($field, $customer->{$field}) }}" class="form-control @error($field) is-invalid @enderror" @required(in_array($field, ['first_name', 'last_name', 'phone']))>@error($field)<span class="form-field__error">{{ $message }}</span>@enderror</label>
                    @endforeach
                    <label class="form-field form-grid__wide"><span class="form-field__label">Adresse</span><textarea name="address" rows="3" class="form-control @error('address') is-invalid @enderror">{{ old('address', $customer->address) }}</textarea>@error('address')<span class="form-field__error">{{ $message }}</span>@enderror</label>
                </div></div>
            </section>
