    ...
    <div class="form-page">
        ...
        <form method="POST" action="{{ $plot->exists ? route('plots.update', $plot) : route('plots.store') }}" class="form-page__content">
            ...
            <section class="form-section">
                ...
                <div class="form-section__body"><div class="form-grid">@if($plot->exists)<x-auth-input name="reference" label="Référence unique" :value="old('reference', $plot->reference)" readonly />@endif<x-auth-input name="plot_number" label="Numéro" :value="old('plot_number', $plot->plot_number)" required /><label class="form-field"><span class="form-field__label">Avenue <span class="form-required">*</span></span><select name="avenue_id" class="form-select">@foreach($avenues as $avenue)<option value="{{ $avenue->id }}" @selected((string) old('avenue_id', $plot->avenue_id) === (string) $avenue->id)>{{ $avenue->neighborhood->name }} · {{ $avenue->name }}</option>@endforeach</select>@error('avenue_id')<span class="form-field__error">{{ $message }}</span>@enderror</label><x-auth-input name="cadastral_reference" label="Référence cadastrale" :value="old('cadastral_reference', $plot->cadastral_reference)" /></div></div>
            </section>
            <section class="form-section">
                <div class="form-section__header"><span class="form-section__number">02</span><div><h2>Caractéristiques physiques</h2><p>Dimensions connues de la parcelle. Le prix est défini lors de la souscription.</p></div></div>
                <div class="form-section__body"><div class="form-grid"><x-auth-input name="surface_area" label="Superficie (m²)" type="number" step="0.01" :value="old('surface_area', $plot->surface_area)" /><x-auth-input name="width" label="Largeur (m)" type="number" step="0.01" :value="old('width', $plot->width)" /><x-auth-input name="length" label="Longueur (m)" type="number" step="0.01" :value="old('length', $plot->length)" /></div></div>
            </section>
            <section class="form-section">
