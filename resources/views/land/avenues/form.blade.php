<x-layouts.app :title="$avenue->exists ? 'Modifier l’avenue' : 'Nouvelle avenue'">
    <div class="form-page">
        <header class="resource-heading">
            ...
            <a href="{{ route('avenues.index') }}" class="btn btn-outline-secondary resource-button">Retour aux avenues</a>
        </header>
        <form method="POST" action="{{ $avenue->exists ? route('avenues.update', $avenue) : route('avenues.store') }}" class="form-page__content">
            ...
            <div class="form-actions"><a href="{{ route('avenues.index') }}" class="btn btn-outline-secondary resource-button">Annuler</a><button class="btn btn-app-primary resource-button" type="submit">{{ $avenue->exists ? 'Enregistrer les modifications' : 'Créer l’avenue' }}</button></div>
        </form>
    </div>
</x-layouts.app>
